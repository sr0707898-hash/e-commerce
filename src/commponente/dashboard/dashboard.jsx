  ...
  return (
    <div className="min-h-screen bg-[#f6f8fb] text-slate-900">
      ...
      <main className="lg:pl-64">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-slate-200 bg-[#f6f8fb]/90 px-5 backdrop-blur md:px-8">
        ...
        <div id="overview" className="mx-auto max-w-360 space-y-6 p-5 md:p-8">
          <section className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-sm text-slate-500">Here is what is happening with your store today.</p><h2 className="mt-1 text-2xl font-bold tracking-tight text-slate-950">Store overview</h2><p className="mt-1 text-xs text-slate-400">{products.length} products in your private catalog</p></div><div className="flex gap-2"><button onClick={() => setShowProductForm(true)} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-600 shadow-sm"><FiPlus className="text-orange-500" /> Add product</button><button onClick={() => setRange(range === "Last 7 days" ? "Last 30 days" : "Last 7 days")} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm font-medium text-slate-600 shadow-sm">{range}<FiChevronDown /></button></div></section>
